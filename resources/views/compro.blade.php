<!DOCTYPE html>
<html lang="{{ session('app_lang', request('lang', 'id')) }}" class="scroll-smooth">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Guitarclassbynde — {{ session('app_lang', request('lang', 'id')) === 'en' ? 'Master Guitar 10x Faster' : 'Jago Main Gitar 10x Lebih Cepat' }}</title>
    <meta name="description" content="{{ session('app_lang', request('lang', 'id')) === 'en' ? 'Master guitar with Nde\'s proven curriculum, 1-on-1 coaching, HD video lessons, and interactive practice tools.' : 'Kuasai gitar dengan kurikulum terstruktur dari Nde, coaching 1-on-1, video materi HD, dan alat latihan interaktif.' }}">
...
<body class="bg-[#08080a] text-gray-100 antialiased relative selection:bg-blue-600 selection:text-white overflow-x-hidden" x-data="{ mobileMenuOpen: false }">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div class="lg:col-span-7 space-y-6 text-left">
                ...
                <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-bold uppercase tracking-wider">
                    ...
                    <span>{{ $isEn ? 'Exclusive Guitar Mentorship Program' : 'Program Mentorship Gitar Eksklusif' }}</span>
                </div>
                ...
                <p class="text-gray-300 text-sm sm:text-base leading-relaxed max-w-2xl">
                    {{ $isEn ? 'Stop wasting months on confusing YouTube tutorials. Access Nde\'s proven step-by-step video curriculum, 1-on-1 live video coaching, and interactive practice tools in one unified platform.' : 'Stop buang-buang waktu berbulan-bulan nonton tutorial YouTube yang bikin bingung. Dapatkan kurikulum video terstruktur dari Nde, coaching video call 1-on-1, dan tools latihan interaktif dalam satu platform.' }}
                </p>
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-2">
                    <a href="{{ $isLoggedIn ? $lmsUrl : url('/registerclass') }}" class="px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold rounded-2xl text-sm shadow-xl shadow-blue-600/30 transition-all hover:scale-105 flex items-center justify-center gap-3">
                        <span>{{ $isLoggedIn ? ($isEn ? 'Enter Student LMS' : 'Masuk ke LMS Murid') : ($isEn ? 'Claim Your Access Now' : 'Daftar & Ambil Akses Sekarang') }}</span>
                ...
                <div class="pt-6 border-t border-white/10 flex flex-wrap items-center gap-6 text-xs text-gray-400">
                    ...
                    <div class="flex items-center gap-1.5 text-emerald-400 font-bold">
                        ...
                        <span>{{ $isEn ? '100% Guaranteed Results' : '100% Garansi Hasil' }}</span>
                    </div>
                </div>
            </div>
                ...
                <div class="glass-panel p-2 relative overflow-hidden shadow-2xl group">
                    <div class="relative rounded-2xl overflow-hidden aspect-video sm:aspect-square bg-zinc-950">
                        ...
                        <div class="absolute bottom-4 left-4 right-4 bg-zinc-950/80 border border-white/10 rounded-xl p-4 backdrop-blur-md flex items-center justify-between">
                            <div>
                                <div class="text-[10px] text-blue-400 font-bold uppercase tracking-wider">{{ $isEn ? 'Mentored by Nde' : 'Bimbingan Langsung Nde' }}</div>
                                <div class="text-xs font-bold text-white">{{ $isEn ? 'Personalized 1-on-1 Feedback Session' : 'Sesi Feedback 1-on-1 Privat' }}</div>
                            </div>
    ...
    <section class="py-8 bg-zinc-950/80 border-y border-white/5 relative z-10">
        <div class="max-w-7xl mx-auto px-4 lg:px-8 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            <div>
                ...
                <div class="text-xs text-gray-400 font-bold uppercase tracking-wider mt-1">{{ $isEn ? 'HD Video Lessons' : 'Video Materi HD' }}</div>
            </div>
            <div>
                <div class="font-display text-4xl sm:text-5xl text-blue-400">1-on-1</div>
                <div class="text-xs text-gray-400 font-bold uppercase tracking-wider mt-1">{{ $isEn ? 'Live Coaching Calls' : 'Sesi Coaching Live' }}</div>
            </div>
            <div>
                <div class="font-display text-4xl sm:text-5xl text-purple-400">5 Suite</div>
                <div class="text-xs text-gray-400 font-bold uppercase tracking-wider mt-1">{{ $isEn ? 'Interactive Practice Tools' : 'Tools Latihan Interaktif' }}</div>
            </div>
            <div>
                <div class="font-display text-4xl sm:text-5xl text-emerald-400">∞</div>
                <div class="text-xs text-gray-400 font-bold uppercase tracking-wider mt-1">{{ $isEn ? 'Lifetime LMS Access' : 'Akses LMS Seumur Hidup' }}</div>
            </div>
        </div>
    </section>
    ...
    <section id="features" class="py-20 px-4 lg:px-8 max-w-7xl mx-auto relative z-10 space-y-12">
        <div class="text-center space-y-3">
            <span class="text-xs font-bold text-blue-400 uppercase tracking-widest block">{{ $isEn ? 'The Ultimate Learning Ecosystem' : 'Ekosistem Belajar Gitar Terlengkap' }}</span>
            <h2 class="font-display text-4xl sm:text-5xl text-white tracking-wide uppercase">
                {{ $isEn ? '3 Core Pillars of' : '3 Pilar Utama' }} <span class="text-blue-500">Guitarclassbynde</span>
            </h2>
            <p class="text-gray-400 text-xs sm:text-sm max-w-xl mx-auto">
                {{ $isEn ? 'The most effective guitar learning system combining video flexibility, 1-on-1 live coaching, and song tutorials.' : 'Sistem belajar gitar paling efektif yang menggabungkan fleksibilitas video, coaching live 1-on-1, dan tutorial lagu.' }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="glass-panel p-8 space-y-5 border-blue-500/30 hover:border-blue-500/50 transition-all duration-300 relative overflow-hidden group">
                ...
                <div>
                    <span class="text-[10px] font-bold text-blue-400 uppercase tracking-widest block mb-1">{{ $isEn ? 'Pillar 1 • Lifetime Access' : 'Pilar 1 • Akses Seumur Hidup' }}</span>
                    <h3 class="font-display text-3xl text-white">{{ $isEn ? 'Video Lessons 24/7' : 'Video Materi 24/7' }}</h3>
                </div>
                <p class="text-gray-400 text-xs sm:text-sm leading-relaxed">
                    {{ $isEn ? 'Access structured HD video modules that you can learn anytime and anywhere at your own pace across all devices.' : 'Akses modul video HD terstruktur yang bisa kamu pelajari kapan saja dan di mana saja sesuai kecepatanmu sendiri.' }}
                </p>
                <div class="pt-2 text-xs font-bold text-blue-400 flex items-center gap-1.5">
                    <i class="fa-solid fa-circle-check text-emerald-400"></i>
                    <span>{{ $isEn ? 'Unlimited Rewatch Access' : 'Akses Nonton Ulang Tanpa Batas' }}</span>
                </div>
            </div>
            ...
            <div class="glass-panel p-8 space-y-5 border-purple-500/30 hover:border-purple-500/50 transition-all duration-300 relative overflow-hidden group">
                ...
                <div>
                    <span class="text-[10px] font-bold text-purple-400 uppercase tracking-widest block mb-1">{{ $isEn ? 'Pillar 2 • On-Website Video Call' : 'Pilar 2 • Video Call di Web' }}</span>
                    <h3 class="font-display text-3xl text-white">1-on-1 Live Coaching</h3>
                </div>
                <p class="text-gray-400 text-xs sm:text-sm leading-relaxed">
                    {{ $isEn ? 'Live 1-on-1 video call coaching inside the website with Nde for real-time technique & finger placement correction.' : 'Sesi coaching video call 1-on-1 langsung di dalam website bersama Nde untuk koreksi teknik & posisi jari secara real-time.' }}
                </p>
                <div class="pt-2 text-xs font-bold text-purple-400 flex items-center gap-1.5">
                    <i class="fa-solid fa-circle-check text-emerald-400"></i>
                    <span>{{ $isEn ? 'Integrated Booking Schedule' : 'Jadwal Booking Terintegrasi' }}</span>
                </div>
            </div>
            ...
            <div class="glass-panel p-8 space-y-5 border-emerald-500/30 hover:border-emerald-500/50 transition-all duration-300 relative overflow-hidden group">
                ...
                <div>
                    <span class="text-[10px] font-bold text-emerald-400 uppercase tracking-widest block mb-1">{{ $isEn ? 'Pillar 3 • Intermediate Tier' : 'Pilar 3 • Tingkat Intermediate' }}</span>
                    <h3 class="font-display text-3xl text-white">{{ $isEn ? 'Song Tutorial Library' : 'Pustaka Tutorial Lagu' }}</h3>
                </div>
                <p class="text-gray-400 text-xs sm:text-sm leading-relaxed">
                    {{ $isEn ? 'Exclusive song breakdown library for Intermediate Students to master popular song arrangements, strumming, and advanced chords.' : 'Pustaka bedah lagu eksklusif untuk Murid Intermediate agar menguasai aransemen lagu populer, strumming, dan chord tingkat lanjut.' }}
                </p>
                <div class="pt-2 text-xs font-bold text-emerald-400 flex items-center gap-1.5">
                    <i class="fa-solid fa-circle-check text-emerald-400"></i>
                    <span>{{ $isEn ? 'Regular Song Updates' : 'Update Lagu Berkala' }}</span>
                </div>
            </div>
    ...
    <section class="py-20 px-4 lg:px-8 max-w-6xl mx-auto relative z-10 space-y-12">
        <div class="text-center space-y-3">
            <span class="text-xs font-bold text-blue-400 uppercase tracking-widest block">{{ $isEn ? 'Why Most Guitar Learners Fail' : 'Mengapa Banyak Orang Gagal Belajar Gitar' }}</span>
            <h2 class="font-display text-4xl sm:text-5xl text-white tracking-wide uppercase">
                {{ $isEn ? 'The Old Way vs' : 'Cara Lama Vs' }} <span class="text-blue-500">{{ $isEn ? 'The Nde System' : 'Sistem Nde' }}</span>
            </h2>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
            ...
            <div class="glass-panel p-8 border-red-500/20 bg-red-500/5 flex flex-col justify-between space-y-6 hover:border-red-500/40 transition">
                <div class="space-y-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="font-display text-2xl text-white tracking-wide uppercase">{{ $isEn ? 'The Frustrating Old Way' : 'Cara Lama Yang Bikin Frustrasi' }}</h3>
                        </div>
                        <span class="px-2.5 py-1 bg-red-500/10 border border-red-500/20 text-red-400 text-[10px] font-extrabold uppercase rounded-full tracking-wider">{{ $isEn ? 'High Risk' : 'Risiko Tinggi' }}</span>
                    </div>
                    ...
                    <ul class="space-y-4 text-xs sm:text-sm text-gray-400">
                        <li class="flex items-start gap-3">
                            ...
                            <span class="leading-relaxed">{{ $isEn ? 'Watching random YouTube tutorials with no clear structure or path.' : 'Nonton tutorial YouTube acak tanpa struktur atau alur yang jelas.' }}</span>
                        </li>
                        <li class="flex items-start gap-3">
                            <i class="fa-solid fa-circle-xmark text-red-400 text-sm mt-0.5 shrink-0"></i>
                            <span class="leading-relaxed">{{ $isEn ? 'No feedback from a mentor, leading to bad finger habits and pain.' : 'Tidak ada umpan balik dari mentor, bikin kebiasaan jari salah dan pegal.' }}</span>
                        </li>
                        <li class="flex items-start gap-3">
                            <i class="fa-solid fa-circle-xmark text-red-400 text-sm mt-0.5 shrink-0"></i>
                            <span class="leading-relaxed">{{ $isEn ? 'Stuck on basic chord switching for months without progress.' : 'Stuck di perpindahan chord dasar berbulan-bulan tanpa kemajuan.' }}</span>
                        </li>
                    </ul>
                </div>
                ...
                <div class="pt-4 border-t border-red-500/10 flex items-center gap-2 text-xs font-bold text-red-400/80">
                    ...
                    <span>{{ $isEn ? '90% Quit Within 3 Months' : '90% Menyerah Dalam 3 Bulan' }}</span>
                </div>
            </div>
            ...
            <div class="glass-panel p-8 border-emerald-500/30 bg-emerald-500/5 flex flex-col justify-between space-y-6 hover:border-emerald-500/50 transition">
                <div class="space-y-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="font-display text-2xl text-white tracking-wide uppercase">{{ $isEn ? 'The Guitarclassbynde System' : 'Sistem Guitarclassbynde' }}</h3>
                        </div>
                        <span class="px-2.5 py-1 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-[10px] font-extrabold uppercase rounded-full tracking-wider">{{ $isEn ? 'Recommended' : 'Rekomendasi Utama' }}</span>
                    </div>
                    ...
                    <ul class="space-y-4 text-xs sm:text-sm text-gray-300 font-medium">
                        <li class="flex items-start gap-3">
                            ...
                            <span class="leading-relaxed">{{ $isEn ? 'Structured 100% step-by-step video curriculum from zero to advanced.' : 'Kurikulum video terstruktur 100% dari nol hingga tingkat lanjut.' }}</span>
                        </li>
                        <li class="flex items-start gap-3">
                            <i class="fa-solid fa-circle-check text-emerald-400 text-sm mt-0.5 shrink-0"></i>
                            <span class="leading-relaxed">{{ $isEn ? 'Direct 1-on-1 live video coaching calls with Nde for instant fixes.' : 'Sesi video call coaching 1-on-1 langsung dengan Nde untuk perbaikan instan.' }}</span>
                        </li>
                        <li class="flex items-start gap-3">
                            <i class="fa-solid fa-circle-check text-emerald-400 text-sm mt-0.5 shrink-0"></i>
                            <span class="leading-relaxed">{{ $isEn ? 'Built-in 5 Suite Practice Tools (Tuner, Metronome, Chord/Scale, Quiz).' : '5 Tools Latihan Interaktif bawaan (Tuner, Metronom, Chord/Scale Visualizer).' }}</span>
                        </li>
                    </ul>
                </div>
                ...
                <div class="pt-4 border-t border-emerald-500/10 flex items-center gap-2 text-xs font-bold text-emerald-400">
                    <i class="fa-solid fa-shield-halved"></i>
                    <span>{{ $isEn ? '98% Success Rate Guaranteed' : 'Garansi Tingkat Keberhasilan 98%' }}</span>
                </div>
            </div>
    ...
    <section id="tools" class="py-20 px-4 lg:px-8 max-w-7xl mx-auto relative z-10 space-y-12">
        <div class="text-center space-y-3">
            <span class="text-xs font-bold text-purple-400 uppercase tracking-widest block">{{ $isEn ? 'Integrated Practice Suite' : 'Suite Latihan Interaktif' }}</span>
            <h2 class="font-display text-4xl sm:text-5xl text-white tracking-wide uppercase">
                5 {{ $isEn ? 'Interactive Practice Tools Included' : 'Tools Latihan Interaktif Terpasang' }}
            </h2>
            <p class="text-gray-400 text-xs sm:text-sm max-w-xl mx-auto">
                {{ $isEn ? 'No need to buy external tuner or metronome apps. All tools are built directly inside your student dashboard.' : 'Tidak perlu beli aplikasi tuner atau metronom terpisah. Semua alat sudah terpasang langsung di dashboard murid kamu.' }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="glass-panel p-6 space-y-3 border-blue-500/20 hover:border-blue-500/40 transition">
                ...
                <p class="text-gray-400 text-xs leading-relaxed">{{ $isEn ? 'Mic pitch detection for accurate guitar tuning visual feedback.' : 'Deteksi nada mikrofon untuk visual setem gitar yang akurat.' }}</p>
            </div>
            ...
            <div class="glass-panel p-6 space-y-3 border-emerald-500/20 hover:border-emerald-500/40 transition">
                ...
                <h3 class="font-display text-2xl text-white">{{ $isEn ? 'Precision Metronome' : 'Metronom Presisi' }}</h3>
                <p class="text-gray-400 text-xs leading-relaxed">{{ $isEn ? 'Tap tempo, time signatures, and custom beat sound engines.' : 'Tap tempo, birama rhythm, dan mesin suara ketukan kustom.' }}</p>
            </div>
            ...
            <div class="glass-panel p-6 space-y-3 border-purple-500/20 hover:border-purple-500/40 transition">
                ...
                <p class="text-gray-400 text-xs leading-relaxed">{{ $isEn ? 'Interactive Rosewood fretboard with audio strum audio playback.' : 'Fretboard Rosewood interaktif lengkap dengan audio petikan chord.' }}</p>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row justify-center gap-6 max-w-4xl mx-auto">
            ...
            <div class="glass-panel p-6 space-y-3 border-cyan-500/20 hover:border-cyan-500/40 transition flex-1">
                ...
                <p class="text-gray-400 text-xs leading-relaxed">{{ $isEn ? 'Master Pentatonic, Blues, and Major scales with ascending audio.' : 'Kuasai tangga nada Pentatonik, Blues, dan Major dengan audio tangga nada.' }}</p>
            </div>
            ...
            <div class="glass-panel p-6 space-y-3 border-rose-500/20 hover:border-rose-500/40 transition flex-1">
                ...
                <p class="text-gray-400 text-xs leading-relaxed">{{ $isEn ? 'Interactive audio ear pitch quiz and fretboard note memory game.' : 'Kuis latihan pendengaran nada dan ingatan letak not fretboard.' }}</p>
            </div>
        </div>
    </section>
                ...
                <div class="glass-panel p-8 flex flex-col justify-between relative overflow-hidden transition-all duration-300 {{ $isFeatured ? 'border-blue-500/50 shadow-[0_0_50px_rgba(59,130,246,0.2)] md:-translate-y-2' : 'border-white/10' }}">
                    ...
                    @if($isFeatured)
                        <div class="absolute top-0 right-0 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[10px] font-extrabold px-4 py-1.5 rounded-bl-2xl uppercase tracking-wider">
                            {{ $isEn ? 'Best Value & Recommended' : 'Paket Paling Populer & Direkomendasikan' }}
                        </div>
                    @endif
                    ...
                    <div class="space-y-6">
                        ...
                        @php
                            ...
                            $pricingUnit = $isTicketPkg ? ($isEn ? '/ 1x session' : '/ 1x sesi') : ($isEn ? '/ lifetime' : '/ seumur hidup');
                        @endphp
                        <div>
                        ...
                        @if($userHasLmsAccess && !$isCoachingPkg)
                            <a href="{{ route('lms.dashboard') }}"
                               class="w-full py-4 rounded-2xl font-bold text-xs uppercase tracking-wider text-center flex items-center justify-center gap-2 shadow-lg transition-all bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-600/30">
                                <span>{{ $isEn ? 'Access Your Course' : 'Akses Kelas Kamu' }}</span>
                        ...
                        @else
                            <a href="{{ $isLoggedIn ? route('kelas.buy',$lessonId).'?package_id='.$pkg->id.'&package_qty=1' : route('register').'?package_id='.$pkg->id.'&package_qty=1' }}"
                               class="w-full py-4 rounded-2xl font-bold text-xs uppercase tracking-wider text-center flex items-center justify-center gap-2 shadow-lg transition-all {{ $isFeatured ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-blue-600/30 hover:scale-105' : 'bg-zinc-900 border border-white/10 hover:bg-white/5 text-white' }}">
                                <span>{{ $isEn ? 'Get Enrolled Now' : 'Ambil Akses Sekarang' }}</span>
    ...
    <section id="faq" class="py-20 px-4 lg:px-8 max-w-5xl mx-auto space-y-12 relative z-10">
        <div class="text-center space-y-3">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-bold uppercase tracking-widest">
                {{ $isEn ? 'Got Questions?' : 'Ada Pertanyaan?' }}
            </div>
            <h2 class="font-display text-4xl sm:text-5xl text-white tracking-wide uppercase">
                {{ $isEn ? 'Frequently Asked' : 'Pertanyaan Yang' }} <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">{{ $isEn ? 'Questions' : 'Sering Diajukan' }}</span>
            </h2>
            <p class="text-gray-400 text-xs sm:text-sm max-w-xl mx-auto">
                {{ $isEn ? 'Everything you need to know about Guitarclassbynde, course access, practice tools, and 1-on-1 coaching.' : 'Segala hal yang perlu kamu ketahui tentang Guitarclassbynde, akses kelas, tools latihan, dan coaching 1-on-1.' }}
            </p>
        </div>
        ...
        <div class="text-center pt-2">
            <a href="{{ route('faq') }}" class="inline-flex items-center gap-2.5 py-3 px-8 rounded-full bg-white/5 border border-white/10 hover:border-blue-500/50 hover:bg-blue-500/10 text-xs font-bold text-gray-300 hover:text-white transition-all shadow-md group">
                <span>{{ $isEn ? 'View All FAQs' : 'Lihat Semua FAQ' }}</span>
        ...
        <div class="glass-panel p-10 sm:p-16 text-center space-y-8 relative overflow-hidden bg-gradient-to-br from-blue-900/30 via-zinc-950 to-indigo-900/20 border-blue-500/30">
            <div class="max-w-2xl mx-auto space-y-4">
                ...
                <p class="text-gray-300 text-xs sm:text-sm leading-relaxed">
                    {{ $isEn ? 'Join over 1,200+ students who have transformed their guitar playing with Nde\'s direct mentorship.' : 'Bergabunglah bersama 1.200+ murid yang telah mentransformasi permainan gitar mereka bersama bimbingan langsung dari Nde.' }}
                </p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                <a href="{{ url('/registerclass') }}" class="px-9 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold rounded-2xl text-sm shadow-xl shadow-blue-600/30 transition-all hover:scale-105 flex items-center justify-center gap-3">
                    <span>{{ $isEn ? 'Get Instant Enrollment Access' : 'Dapatkan Akses Kelas Sekarang' }}</span>
    ...
    <footer class="py-8 text-center text-xs text-gray-400 border-t border-white/5 relative z-10">
        <p>&copy; {{ date('Y') }} GUITARCLASSBYNDE. {{ $isEn ? 'All rights reserved.' : 'Hak cipta dilindungi undang-undang.' }} Developed by <strong class="text-white font-bold me-1">WardellTech</strong></p>
    </footer>
